...
public:
    bool doesWordExist(vector<vector<char>> &board, string word, int i, int j, int x, vector<vector<bool>> &visited){
        ...
        visited[i][j] = true;
        bool ans =  doesWordExist(board, word, i+1, j, x+1, visited) || doesWordExist(board, word, i-1, j, x+1, visited) || doesWordExist(board, word, i, j-1, x+1, visited) || doesWordExist(board, word, i, j+1, x+1, visited);
        visited[i][j] = false;
        return ans;
    }
