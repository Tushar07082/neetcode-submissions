...
public:
    TreeNode* getTreeRoot(vector<int> &preorder, vector<int> &inorder, int start, int end, int &preOrderIdx, unordered_map <int,int> &inorderIdx){
        ...
        ans->left = getTreeRoot(preorder, inorder, start, rootIdx-1, preOrderIdx, inorderIdx);
        ans->right = getTreeRoot(preorder, inorder, rootIdx+1, end, preOrderIdx, inorderIdx);
        return ans;
    }
